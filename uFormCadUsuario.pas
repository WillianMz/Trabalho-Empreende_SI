...
begin
  ...
  try
    try
      ...
      on E:Exception do
        begin
          ShowMessage('Se você entender este erro, você está no curso certo. Erro: '+e.Message);
        end;
    end;
  finally
    Response.Free();
...
begin
  try
    //True por padrão, caso o usuário esqueça
...
begin
  ...
  if edtCelular.Text = '' then
    begin
      ShowMessage('Informe o Nº do seu celular');
      edtCelular.SetFocus;
      exit;
    end;
   ...
   if (Length(edtCelular.Text) < 11) then
    begin
      ShowMessage('O Número de celular deve conter 11 digítos incluindo o DDD + Número!');
      edtCelular.SetFocus;
      exit;
    end;
